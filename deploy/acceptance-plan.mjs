...
const flags = ['thirteen', 'thirteen_pwa', 'umo_pwa', 'social_pwa', 'voice'];
...
export function classify({frontendPaths,backendPaths,readFrontend,forceFull=false,baselineValid=true}) {
 const plan={deploy:false,thirteen:false,thirteen_pwa:false,umo_pwa:false,social_pwa:false,voice:false,reasons:[]};
 ...
 for(const p of frontendPaths){
  ...
  if(p==='public/sw.js'){
   ...
   else mark('service-worker-behavior','thirteen_pwa','umo_pwa','social_pwa','voice');
  } else if(p==='src/lib/pwa-version.ts'){
   const strip=s=>s.replace(/export const PWA_VERSION = 'v\d+';/,"export const PWA_VERSION = 'VERSION';");
   if(strip(readFrontend(p,true))===strip(readFrontend(p,false)))mark('cache-version-constant-only');
   else mark('shared-pwa-bootstrap','thirteen_pwa','umo_pwa','social_pwa','voice');
  } else if(/^public\/thirteen-social\/|^src\/app\/thirteen-social\//.test(p))mark('thirteen-social:'+p,'social_pwa');
  ...
  else if(/^src\/app\/(layout\.tsx|globals\.css|pwa-v5\.css)|^src\/components\/PwaProvider\.tsx$/.test(p))mark('shared-shell:'+p,'thirteen_pwa','umo_pwa','social_pwa','voice');
  else if(p==='.github/workflows/deploy.yml'||p==='deploy/acceptance-plan.mjs'||(p==='tools/pwa/social-acceptance.mjs'||p==='tools/pwa/chrome-session.mjs')||/^test\/deploy\//.test(p))plan.reasons.push('pipeline-only:'+p);
...
export function buildPlan(frontend,backend,baselineFront,baselineBack,forceFull=false){
 ...
 let backendChanged=true;
 try{backendChanged=!shaPattern.test(baselineBack)||git(backend,['rev-parse','HEAD']).trim()!==baselineBack;}catch{}
 return {...classify({frontendPaths,backendPaths,readFrontend,forceFull,baselineValid}),backend_changed:backendChanged,backend_build:forceFull||!baselineValid||backendChanged,baselineFront,baselineBack,frontendPaths,backendPaths};
}
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href){
 ...
 if(process.env.GITHUB_OUTPUT)appendFileSync(process.env.GITHUB_OUTPUT,['deploy','backend_build','backend_changed',...flags].map(k=>`${k}=${plan[k]}`).join('\n')+'\n');
 if(process.env.GITHUB_STEP_SUMMARY)appendFileSync(process.env.GITHUB_STEP_SUMMARY,`## Deployment scope\n\nProduction frontend: ${baselineFront}\n\nProduction backend: ${baselineBack}\n\n| Check | Selection |\n| --- | --- |\n${['deploy','backend_build','backend_changed',...flags].map(k=>`| ${k} | ${plan[k]?'run':'skip'} |`).join('\n')}\n\nReasons:\n\n${plan.reasons.map(r=>'- '+r).join('\n')}\n`);
