 ...
 *
 * **6 格要塞五行,余量只有 0.33 格 —— 改任何一个数之前先跑一遍自检。**
 *
 * 几个尺寸是联动的:棋子是竖长的(高 1.31 格,锚点在底座,所以身子往上伸 1.07 格),
 * 头像是直径 1.7 格的圆,基地底边还要让开 0.34 格的金框(可用到 5.66)。
 * 早先按"棋子行"往下摆,结果棋子的身子插进了分数条;把骰子放在 5.25 又压到金框上
 * (截图里的"骰子超出位置")。
 *
 * 现在的分配(单位:格,含各自的高度):
 *   头像  0.15~1.85   分数条 2.15~3.00   棋子 3.30~4.61   骰子 4.86~5.64
 */
export const PANEL = {
  avatarRow: 1.0,
  /** 名字和头像同一行,在它右侧 */
