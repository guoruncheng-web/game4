...
export class Stage {
  ...
  private setupPylons() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      group.position.set(
        (i % 2 === 0 ? -1 : 1) * (16 + Math.random() * 7),
        (Math.random() - 0.5) * 12,
        -40 - i * 42 - Math.random() * 20,
      );
  ...
  update(dt: number, playerX: number, playerY: number) {
    ...
    for (const pylon of this.pylons) {
      ...
      if (pylon.position.z > this.camera.position.z + 20) {
        pylon.position.z -= 460;
        pylon.position.x = (Math.random() < 0.5 ? -1 : 1) * (16 + Math.random() * 7);
        pylon.position.y = (Math.random() - 0.5) * 12;
      }
    }
