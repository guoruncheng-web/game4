...
联机走的**不是**本文 §1 原定的 WebRTC P2P。项目在实现途中从 Vercel 迁到了自托管
(阿里云 + www.gameai.xingzdh.com),有了常驻进程之后 P2P 的理由就不存在了 ——
改成 `server/ws.mjs` 一个独立的 WebSocket 服务中转,顺带消掉了 P2P 那 10~20% 的
NAT 穿透失败率。本文 §1、§3 描述的信令表和轮询接口**已全部删除**。

现在的形态:
- 全站一条 WebSocket(`src/components/CoopProvider.tsx`),登录即连,邀请在任何页面都能收到
- 独立匹配页 `/neon-strike-2d/lobby`,下拉选人邀请,双方就位后由房主开始
- `coop/bridge.ts` 把 React 侧的连接递给 Phaser 侧,两边互不认识对方的技术栈
- §4 的权威划分、§4.2 的消息表**原样成立**,只是换了条管子

### 已完成

1. 单人模式行为与改造前一致(联机分支全靠 `coopSession` / `isCoop` 短路)
2. 在线、邀请、接受、进同一局
3. 敌机的生成 / 死亡 / 4Hz 漂移校正
4. host 裁决击杀,不会出现「打不死的残影」
5. Boss 同步(生成参数由 host 摇、弹幕只由 host 排、血条单独同步)
6. 道具由 host 决定掉落并广播,捡走后通知对方移除
7. 一方阵亡进观战、下一波复活,两人都倒才结算
8. host 掉线 → guest 直接结算;guest 掉线 → host 独自打完
9. 联机局不写单人排行榜
10. 结算页双人并排 + 合计 + MVP
11. **开局等双方加载完**,并在 Boot 阶段互报进度

### 未完成 / 已知缺口

- **全部靠静态推演,没有真机联测。** 上面每一条都编译通过、逻辑自洽,
  但敌机一致性、Boss 同步、漂移校正、阵亡复活**只有两台设备实际打一局才能确认**。
  这是当前最大的不确定,不要当成「已验证」。
- 断线不能重连回局内:断了就是结束,重连只回大厅。
  要做得让服务端保留房间和权威状态,是另一个量级。
- 道具双方同时碰到会各拿一份(协作模式里无害,刻意没加裁决)。
- 无尽模式没做联机,联机固定走战役。

### 踩过的坑(改这块之前先读)

- **前缀匹配**:跳转守卫写成 `pathname.startsWith('/' + game)`,而点开始时双方都在
  `/neon-strike-2d/lobby` —— 它正好以 `/neon-strike-2d` 开头,被判成「已经在游戏里」,
  表现是**点开始没反应**。要精确比对。
- **新连接不补房间状态**:重连后只收到 ready 和 online,而 online 又刻意跳过在房间里的人,
  于是这个人在服务端「在房间里」、浏览器里却什么都看不到,**而另一个人看得见他**。
- **对象池漏回收**:`peerShots` 没进那个回收循环,64 个池子约 9 秒耗尽,
  表现是「打到一半对方子弹就不出来了」,看着像掉线;卡住的物理体还在参与碰撞,顺带掉帧。
- **20Hz 硬设位置**:画面 60fps,等于每三帧才动一次,看起来一跳一跳。要逐帧插值。
- **状态被清掉之后仍然要用**:`isCoop` 和队友名字都得在开局时另存一份 ——
  `coopSession` 在对方掉线时会被清掉,而那恰恰是最需要这两个值的时候。
- **启动命令散在两处**:`package.json` 的 `ws` 脚本和 systemd 单元,
  改一处没改另一处,表现是线上崩溃重启循环而**本地怎么试都正常**。
