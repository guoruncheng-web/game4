...
## Browser launcher correction

Actions 34825630007 failed NO_CHROME_PORT before navigating to the game; automatic rollback restored v95. This attempt did not measure game startup and cannot be called a game loading failure. Old launcher did not drain/save stderr or detect early exit. Added explicit about:blank, bounded 30s browser readiness (separate from unchanged 30s game budget), HTTP DevTools readiness, spawn/exit diagnostics and isolated process-group cleanup. Workflow preflights Chrome before staging/cutover and uploads its logs. Launcher tests cover stderr backpressure, successful endpoint readiness, exit23 and missing executable; workflow ordering is checked. Local actual Chrome and complete PWA repeat pass: cold4208/cached2411/offline2470ms. Runtime bundle/cache remain v97, unchanged from the prior candidate.
