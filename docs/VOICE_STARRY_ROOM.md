...
重新发布：frontend 062dffd2b00af7a00819fc394652ccf5c0ba87c2，backend 固定 8c1467b41d8cd743df6faa7d535600d93066745a，PWA v90，Actions 34555506949 成功（https://github.com/guoruncheng-web/game4/actions/runs/34555506949）。公网双端 16 项 RTC 检查通过、browserErrors=[]，含点选麦位、声波对双方可见且关麦消失、空白处二次确认下麦/取消保留、刷新仅收听、三次断网恢复。按需计划跳过 4 个未受影响游戏套件（跳过不等于本次通过）。遗留：两端进房首个 `GET /api/voice/rooms/:id` 各返回一次 401（新增失败记录可见），随后轮询恢复；会闪现 “Unauthorized” 并可能触发一次语音断开重连，待修。证据：evidence/voice-controls/production-062dffd/。
