...
状态：RC1–RC4 已于 2026-08-27 部署；RC5 首屏传输优化已完成本地生产验收，等待同日滚动发布。

## 组成

- `/umo`：游客可直接访问的同源 Cocos iframe 宿主页；首页游戏卡保持当前 UMO Figma MainMenu 封面。
- `/umo/game/`：Cocos Creator 3.8.8 Web Mobile release，RC5 为 108 个文件、10,005,252 bytes；固定 Boot 场景 UUID `a773f311-9d5d-405f-bda8-294a1c21626a`。
...
Service Worker 版本为 `v29`。`/umo/` 下的 JS、JSON、CSS、WASM、图片和字体采用“玩过即缓存”；RC2 合并 resources JSON，RC3 缩小 Boot 范围，RC5 以正确 credential mode 并行预载首屏背景并把该 RGB PNG 从 1,289,938 降到 353,468 bytes。首次进入缓存 44 项，安装 PWA 时不主动下载游戏包。
...
- PWA 离线：v29 缓存包含 game index、settings、main bundle 和路由；断网重载 `/umo` 后仍启动编辑器编排的 MainMenu。
- 证据：`evidence/runtime/umo-pwa-rc1/result.json` 与 `umo-pwa-offline.png`。

## 产物追溯

- Cocos RC5 sorted-manifest SHA-256：`6d986f858c9d5e0d314d055c032c7b3f13f18ffee8a810f0b228e71ac8caa1c7`。
...
- Runtime gzip-9 逐文件合计：7,201,522 bytes。

公网部署、域名、TLS、生产状态密钥和 Actions 权限沿用 game4 现有运维流程。RC1–RC4 的 Actions 运行 `33054256627`、`33055893104`、`33058029238`、`33058601766` 均已成功构建并重启双服务；RC5 继续沿用同一幂等流程。
