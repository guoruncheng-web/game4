...
状态：RC1 已于 2026-08-27 部署；RC2 资源请求优化已完成本地生产验收，等待同日滚动发布。

## 组成

- `/umo`：游客可直接访问的同源 Cocos iframe 宿主页；首页游戏卡保持当前 UMO Figma MainMenu 封面。
- `/umo/game/`：Cocos Creator 3.8.8 Web Mobile release，RC2 为 108 个文件、10,942,222 bytes；固定 Boot 场景 UUID `a773f311-9d5d-405f-bda8-294a1c21626a`。
...
Service Worker 版本为 `v26`。`/umo/` 下的 JS、JSON、CSS、WASM、图片和字体采用“玩过即缓存”；RC2 把 resources Bundle 的 JSON 从 139 个以上合并为 1 个，完整运行缓存从 RC1 的 236 项降到 98 项。安装 PWA 时不主动下载游戏包，在线玩过一次后可离线重放已缓存的 MainMenu。
...
- PWA 离线：v26 缓存包含 game index、settings、main bundle 和路由；断网重载 `/umo` 后仍启动编辑器编排的 MainMenu。
- 证据：`evidence/runtime/umo-pwa-rc1/result.json` 与 `umo-pwa-offline.png`。

## 产物追溯

- Cocos RC2 sorted-manifest SHA-256：`6356942f2c9afcaaadf32f50191ee7405b89f7d8f2dfe4266ce8815e2670e88c`。
...
- Runtime gzip-9 逐文件合计：8,138,304 bytes。

公网部署、域名、TLS、生产状态密钥和 Actions 权限沿用 game4 现有运维流程。RC1 的 Actions 运行 `33054256627` 已成功初始化加密状态、构建并重启双服务；RC2 继续沿用同一幂等流程。
