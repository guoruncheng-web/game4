...
本地：前端 lint、生产构建；10 项音频会话/SDK 清理测试（包括权限拒绝/晚返回/撤权/重连清理）、9 项按需部署规则测试；后端 lint、20 项单测、五服务构建通过。独立本地 gateway 17100 + voice 17104 复用平台服务；双端 Chrome 走真实页面、网关、数据库和声网，使用合成麦克风音频，不读取真实环境声音。
