...
const origin = args[0] ?? 'http://127.0.0.1:3000';
const wsOrigin = args[1] ?? origin.replace(/^http/, 'ws');
const password = 'Testpass123';
const username = `test-auth-game-link-${Date.now()}`;
...
const users = await sql`
  ...
  select candidate, ${username}, ${hashPassword(password)}, '🧪', now()
...
const fixtureUid = Number(users[0]?.uid);
assert.ok(Number.isSafeInteger(userId) && /^\d{6}$/.test(String(fixtureUid)));
...
try {
  ...
  assert.equal(identity.uid, fixtureUid);
