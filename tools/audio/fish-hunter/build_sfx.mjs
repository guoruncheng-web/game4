#!/usr/bin/env node
/**
 * 深海捕鱼音效生成 —— 用 rFXGen(raysan5, sfxr 系)出音。
 *
 * 结构和 tools/audio/neon-strike/build_sfx.mjs 完全一样(写 .rfx → 渲染 → 混层 → 归一),
 * 那份的注释解释了为什么要分层、为什么 deltaSlide 必须 ≤ slide,这里不重复。
 *
 * 流程:
 ...
 *   node tools/audio/fish-hunter/build_sfx.mjs --rfxgen /path/to/rfxgen
 *   node tools/audio/fish-hunter/build_sfx.mjs --ssh mac@192.168.64.1   # 在 Mac 上远程渲染
 *
...
const OUT = join(ROOT, 'public/fish-hunter/audio');
...
// ---------------------------------------------------------------- 音效定义
/**
 * gain 是这一层在混音里的相对音量;at 是起始秒数;peak 是最终归一化到的峰值。
 *
 * **包络参数不是秒。** rFXGen(sfxr 系)里 attack/sustain/decay 的实际时长是
 *   t = 参数² × 100000 采样 ÷ 44100 ≈ 参数² × 2.27 秒
 * 平方关系,所以 0.10 只有 23ms、0.30 有 204ms。第一版按"秒"来填,
 * 出来的 fire 是 30ms、coin 是 10ms —— 短到听着像爆音。查表:
 *   0.10→23ms  0.14→44ms  0.20→91ms  0.26→153ms  0.30→204ms  0.40→363ms
 *
 * 这一组的响度关系是刻意排的(ART.md §6):
 *   coin(0.30) < fire(0.42) < deny(0.45) < pop(0.55) < catch(0.62) < jackpot(0.80)
 * fire 和 coin 会被高频重复触发 —— 一秒 4.5 发炮、一次入账最多 6 枚金币,
 * 它们的峰值必须压住,否则连打二十秒就只剩噪音了。
 */
const SOUNDS = [
  {
    name: 'fire', peak: 0.42,
    // 发射:短促的气压推送。方波下扫给"咻",底下一层极短 sub 给推力。
    // 决定音色的是**短** —— 220ms 一发,尾巴稍长就会糊成一片
    layers: [
      { gain: 1, params: { waveType: SQUARE, startFrequency: 0.44, minFrequency: 0.10, slide: -0.34, deltaSlide: -0.34,
        sustainTime: 0.10, sustainPunch: 0.35, decayTime: 0.20, squareDuty: 0.38, dutySweep: -0.20,
        lpfCutoff: 0.70, lpfResonance: 0.18, hpfCutoff: 0.10 } },
      { gain: 0.55, params: { waveType: SINE, startFrequency: 0.15, slide: -0.24, deltaSlide: -0.24,
        sustainTime: 0.09, sustainPunch: 0.40, decayTime: 0.18, lpfCutoff: 0.30 } },
    ],
  },
  {
    name: 'pop', peak: 0.55,
    // 网入水炸开:闷响,不是爆炸。低通压得很低 + 一层 sub,
    // 高频留太多会变成"打铁",而这是一张网撒在水里
    layers: [
      { gain: 1, params: { waveType: NOISE, startFrequency: 0.34, slide: -0.22, deltaSlide: -0.22,
        attackTime: 0.02, sustainTime: 0.14, sustainPunch: 0.45, decayTime: 0.32,
        lpfCutoff: 0.30, lpfCutoffSweep: -0.14, lpfResonance: 0.12, hpfCutoff: 0.03 } },
      { gain: 0.70, params: { waveType: SINE, startFrequency: 0.12, slide: -0.16, deltaSlide: -0.16,
        sustainTime: 0.12, sustainPunch: 0.40, decayTime: 0.28, lpfCutoff: 0.26 } },
    ],
  },
  {
    name: 'catch', peak: 0.62,
    // 捕获小鱼:两声上行的清脆点。第二声晚 60ms,做出"叮-咚"的入账感
    layers: [
      { gain: 1, params: { waveType: SINE, startFrequency: 0.52, slide: 0.14, deltaSlide: 0.02,
        sustainTime: 0.12, sustainPunch: 0.30, decayTime: 0.26, lpfCutoff: 0.90 } },
      { gain: 0.70, at: 0.06, params: { waveType: SINE, startFrequency: 0.66, slide: 0.12, deltaSlide: 0.01,
        sustainTime: 0.10, sustainPunch: 0.25, decayTime: 0.28, lpfCutoff: 0.92 } },
      { gain: 0.35, params: { waveType: SQUARE, startFrequency: 0.58, sustainTime: 0.07, decayTime: 0.16,
        squareDuty: 0.28, lpfCutoff: 0.80 } },
    ],
  },
  {
    name: 'jackpot', peak: 0.80,
    // 捕获大鱼:四段上行琶音 + 一层铺底。
    // **必须和 catch 明显不同** —— 大鱼入账是这类游戏的记忆点(ART.md §6),
    // 靠的就是"四声"这个长度差:catch 是一个点,这个是一句话
    layers: [
      ...[0, 1, 2, 3].map((i) => ({
        gain: 1 - i * 0.06, at: i * 0.09,
        params: { waveType: SQUARE, startFrequency: 0.42 + i * 0.09, slide: 0.10, deltaSlide: 0.01,
          sustainTime: 0.12, sustainPunch: 0.35, decayTime: 0.28,
          squareDuty: 0.32, dutySweep: -0.10, lpfCutoff: 0.92 },
      })),
      { gain: 0.45, at: 0.02, params: { waveType: SINE, startFrequency: 0.22, slide: 0.06, deltaSlide: 0.005,
        attackTime: 0.10, sustainTime: 0.28, decayTime: 0.40, lpfCutoff: 0.40 } },
    ],
  },
  {
    name: 'coin', peak: 0.30,
    // 金币落袋:极短极亮。一次捕获最多飞 6 枚、每枚间隔 40ms,
    // 所以它必须又短又轻 —— 长一点就会自己叠成一团嗡嗡声
    layers: [
      { gain: 1, params: { waveType: SINE, startFrequency: 0.74, slide: 0.16, deltaSlide: 0.01,
        sustainTime: 0.08, sustainPunch: 0.20, decayTime: 0.17, lpfCutoff: 1 } },
    ],
  },
  {
    name: 'deny', peak: 0.45,
    // 余额不足:闷、短、往下沉。**不做惩罚感** ——
    // 这游戏不卖金币(DESIGN §0),没钱只是个提示,不该让人觉得被扇了一巴掌
    layers: [
      { gain: 1, params: { waveType: SAW, startFrequency: 0.22, minFrequency: 0.05, slide: -0.20, deltaSlide: -0.20,
        attackTime: 0.03, sustainTime: 0.12, sustainPunch: 0.20, decayTime: 0.30,
        lpfCutoff: 0.24, lpfResonance: 0.28 } },
    ],
  },
];
...
if (ssh) {
  const remote = '/tmp/fish-hunter-rfx';
